...
class HANSampler(object):
    def __init__(self, g, metapath_list, num_neighbors, device="cpu"):
        self.sampler_list = []
    ...
    def sample_blocks(self, seeds):
        block_list = []
        seeds_tensor = torch.Tensor(seeds).to(torch.int64).to(self.device)
        for sampler in self.sampler_list:
            frontier = sampler(seeds)
            # add self loop
            frontier = dgl.remove_self_loop(frontier)
            frontier.add_edges(seeds_tensor, seeds_tensor)
