...
class RelGraphEmbed_Pretrain(nn.Module):
    ...
    def __init__(
        ...
        for ntype in g.ntypes:
            embed = None
            if ntype != "user":
                embed = nn.Linear(g.nodes[ntype].data["features"].shape[1], self.embed_size)
            else:
                embed = nn.Embedding(10000, self.embed_size)
            # nn.init.xavier_uniform_(embed, gain=nn.init.calculate_gain("relu"))
            self.embeds[ntype] = embed
    ...
    def forward(self, block=None):
        ...
        if block is None:
            for ntype in self.embeds:
                if ntype != "user":
                    h[ntype] = self.embeds[ntype](self.g.nodes[ntype].data["features"].to(th.float32))
                else:
                    h[ntype] = self.embeds[ntype](self.g.nodes(ntype))
        else:
            for ntype in self.embeds:
                if ntype != "user":
                    h[ntype] = self.embeds[ntype](block.nodes[ntype].data["features"].to(th.float32))
                else:
...
class EntityClassify(nn.Module):
    def __init__(
        ...
        self.embed_layer = RelGraphEmbed_Pretrain(g, self.h_dim)
...
class EntityCluster(nn.Module):
    def __init__(
        ...
        super(EntityCluster, self).__init__()
        self.g = g
        self.h_dim = h_dim
        self.out_dim = out_dim
        self.rel_names = list(set(g.etypes))
