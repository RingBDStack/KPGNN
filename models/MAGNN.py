...
from utils.metapath import metapath_instance_feat
...
class IntraMetapathAggregation(nn.Module):
    ...
    def forward(self, g, node_feat, edge_feat):
        ...
        # print(f'g.num_dst_nodes: {g.num_dst_nodes}')
...
class MAGNNLayerNtypeSpecific(nn.Module):
    ...
    def forward(self, gs, node_feat, edge_feat_name):
        ...
        # metapath_embeds = []
        # for agg, g in zip(self.intra_metapath_aggs, gs):
        #     metapath_embeds.append(
        #         agg(g, node_feat, g.edata[edge_feat_name]).flatten(start_dim=1)
        #     )
...
class MAGNNMinibatch(nn.Module):
    ...
    def __init__(self, g, ntype, metapaths, in_dims, hidden_dim, embed_size, out_dim, num_heads, encoder, dropout=0.0):
        ...
        #     embed = None
        #     if ntype != "user":
        #         embed = nn.Linear(in_dims[ntype], hidden_dim)
        #     else:
        #         embed = nn.Embedding(in_dims[ntype], embed_size)
        #     # nn.init.xavier_uniform_(embed, gain=nn.init.calculate_gain("relu"))
        #     self.feat_trans[ntype] = embed
    ...
    def forward(self, blocks, node_feats):
        ...
        #     print(f'{ntype}.shape {node_feats[ntype].shape}')
        # hs = {}
        # hs['tweet'] = self.feat_drop(self.feat_trans['tweet'](node_feats['tweet']))
        # hs['user'] = self.feat_drop(self.feat_trans['user'](node_feats['user']))
        # hs['entity'] = self.feat_drop(self.feat_trans['entity'](node_feats['entity']))
        # hs['word'] = self.feat_drop(self.feat_trans['word'](node_feats['word']))
        # hs['hashtag'] = self.feat_drop(self.feat_trans['hashtag'](node_feats['hashtag']))
        hs = {
        ...
        # for ntype in self.feat_trans:
        #     if ntype != "user":
        #         hs[ntype] = self.feat_drop(self.feat_trans[ntype](node_feats[ntype].to(torch.float32)))
        #     else:
        #         hs[ntype] = self.feat_drop(self.feat_trans[ntype](self.g.nodes(ntype)))
